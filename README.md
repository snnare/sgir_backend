...
### 4️⃣ Consolidación de Reportes SRE, Endpoints Offline y Limpieza de CMDB (2 de Junio, 2026)
Se expandieron las capacidades de observabilidad y generación de reportes ejecutivos del backend, optimizando el rendimiento mediante consultas offline y depurando el motor de importación masiva:
*   **Compatibilidad RDBMS + Versión:** Se modificaron los reportes de inventario de bases de datos (PDF y CSV) para mapear e inyectar dinámicamente el nombre del motor junto con su versión (ej. `"MySQL 8"`, `"MongoDB 6"`), manteniendo la correcta tematización visual de los badges del reporte de la UAEMex.
*   **Consolidación de Endpoints Offline:** Se modificó `/assets/csv` para operar sin llamadas por red y se creó el endpoint `/assets/pdf-offline` para generar reportes del estado actual de la CMDB PostgreSQL de forma instantánea sin requerir auto-descubrimiento SSH en caliente.
*   **Reporte de SLA, Incidentes y Uptime (SRE):** Se diseñó la plantilla `sre_sla_uptime_template.html` y se creó el endpoint `GET /sgir/v1/assets/sre-sla-pdf` para reportar métricas de disponibilidad de hardware (integradas con la caché en RAM `LIVE_METRICS_CACHE`) y el histórico de incidentes y alertas registradas en base de datos.
*   **Resumen Global de Políticas:** Se implementó `GET /sgir/v1/crud/politicas-respaldo/resumen-global` que provee una consulta agrupada y relacional de políticas asignadas a bases de datos y servidores de forma directa.
*   **Depuración de Carga Masiva:** Se eliminaron las rutas redundantes de `/import-bulk` (bases de datos, políticas, rutas y asignaciones), unificando todo en la carga única de infraestructura y depurando `import_service.py` para eliminar código de parsing obsoleto.
*   **Organización de Mockups:** Se creó la subcarpeta `app/templates/reports/mockups` para agrupar todas las vistas de prueba y se diseñó la simulación a escala de 50 servidores multi-motor (`general_infrastructure_template_test.html`) y de SLA mensual (`sre_sla_uptime_template_test.html`).
