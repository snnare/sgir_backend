...
2.  **Scheduler de Alta Disponibilidad y Mantenimiento Automático**:
    ...
    *   **Retention Manager**: Tarea diaria a las 3:00 AM para purgar métricas y sesiones de más de 30 días.
    *   **Controles SRE**: Endpoints para `pausar`, `reanudar` y consultar el `status` del motor.
3.  **Gestión de Alertas y Dashboards**:
    ...
    *   **Endpoint de Salud Global**: Una sola petición consolida el conteo total de servidores en estado "Sano", "Crítico" y "Desactualizado" para pintar el Dashboard Principal.
...
Dado que el motor base está completo, el enfoque debe dirigirse a los **Roles y Permisos (Seguridad)** o al refinamiento final del módulo de **Respaldos (Lógica de expiración)**.
